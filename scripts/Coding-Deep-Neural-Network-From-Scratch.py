...
def initialize_parameters(layers_dims):
    """
    Arguments:
    layers_dims -- list or numpy array that has the dimensions of each layer in the network
    Returns:
    parameters -- dictionary that has the weight matrices and the bias vector for each layer
    """
    np.random.seed(1)               # to get consistent output
    parameters = {}                 # initialize parameters dictionary
    L = len(layers_dims)             # number of layers in the network

    for l in range(1, L):           # starts from one coz we dont count input layer
...
def linear_forward(A_prev, W, b):
    ...
    W -- Weight matrix of shape (size of current layer x size of previuos layer)
    b -- bias vector of shape (size of current layer x 1)
    Returns:
...
def linear_activation_forward(A_prev, W, b, activation_fn):
    ...
    W -- Weight matrix of shape (size of current layer x size of previuos layer)
    b -- bias vector of shape (size of current layer x 1)
    activation_fn -- string that specify the activation function to be used: "sigmoid", "tanh", "relu"
    Returns:
    A -- Output of the activation function
    cache -- tuple that stores linear_cache and activation_cache ((A_prev, W, b), Z) to be used in backpropagation
    """
...
def L_model_forward(X, parameters, hidden_layers_activation_fn = "relu"):
    ...
    parameters -- dictionary that contains all the weight matrices and bias vectors for all layers
    hidden_layers_activation_fn -- activation function to be used on hidden layers, string: "tanh", "relu"
    Returns:
...
def L_layer_model(X, Y, layers_dims, learning_rate = 0.01, num_iterations = 3000, print_cost=False, hidden_layers_activation_fn = "relu"):
    """
    Arguments:
